...
(defn- ink-component [revision samples]
  (component/validate-component!
   (-> fixtures/example-ink-component
       (assoc :path/revision revision)
       (assoc-in [:path/geometry :stroke-points]
                 (mapv (fn [index [x y pressure]]
                         {:stroke-point/id [:stroke-point index]
                          :position [x y]
                          :width (* 10.0 pressure)
                          :pressure pressure})
                       (range) samples)))))
...
(deftest deterministic-direct-stroke-and-invalidation-tripwire
  (let [left (ink-component :left/rev-1
                           [[0.0 0.0 0.2] [20.0 0.0 0.6]
                            [30.0 10.0 1.0]])
        right (assoc (ink-component :right/rev-1
        ...
        edited-left (-> left
                        ...
                        (assoc-in [:path/geometry :stroke-points 1 :position]
                                  [21.0 1.0]))
...
(deftest concave-hole-ear-clip-tripwire
  (let [shape (component/validate-component! fixtures/example-shape-component)
    ...
    (is (= :legal-max (:lod mesh)))
    (is (= (mesh-bytes mesh) (mesh-bytes replay)))
    (is (point-in-mesh? mesh [2.0 20.0]))
    (is (not (point-in-mesh? mesh [8.0 8.0])))))

(deftest zoom-lod-and-geometry-dedupe-tripwire
  (let [ink (component/validate-component! fixtures/example-ink-component)
        repainted (assoc-in ink [:path/paint :color] [0.9 0.1 0.2 0.8])
        derivation (tessellation/derive-mesh-set {} [ink repainted] 1.0)]
    (is (= [:legal-min :engine-default :engine-default :legal-max :legal-max]
           (mapv (comp :lod/id tessellation/zoom-lod)
...
(deftest width-move-preserves-pinned-mesh-bytes-tripwire
  (let [ink (component/validate-component! fixtures/example-ink-component)
        shape (component/validate-component! fixtures/example-shape-component)]
