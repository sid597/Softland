(ns app.server.episode.cascade-table-test
  ...
  (:require [app.server.episode.cascade :as cascade]
            [app.server.door.server-jetty :as server]
...
(deftest cascade-table-contract-gates
  ...
  (testing "G5 — row #1 is complete, unique, printable, and vocabulary-bound"
    (let [rows (cascade/rows)
      ...
      (is (= 'app.server.door.server-jetty/run-ambient-autotag!
             (:cascade/handler row)))
  ...
  (testing "G1/G2 — the table preserves autotag identity and replay convergence"
    (let [object-key "chat:cascade-identity"
          ...
          ambient-runtime-var
          (ns-resolve 'app.server.door.server-jetty 'ambient-autotag-runtime)
          fake-read-unit
          (fn [_runtime unit-id]
            (case unit-id
