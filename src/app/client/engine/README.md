...
| [executor.cljc](executor.cljc) | Evaluate one EDN expression language; run records with named capability arguments and explicit loop state; suspend/resume byte continuations; return computation subjects. A program is compiled once per run. A complete run keeps its continuation at the end of its items, so a stream that grows resumes with the new items alone. Evidence: `test/app/client/engine/executor_test.clj`. |
...
Result subjects include resolved pre-loop reads that feed loop state as well as loop reads; unused pre-loop reads stay out (`executor_pending_test/a-loop-subject-includes-the-read-that-initialized-state`). A state-dependent output's subject also names the roots its items reached (`:item-roots`), beside the recipe, never in it. A retained input is admitted when its subject's program and output are the declared producer's, the producer's own roots and the items' roots equal the producer record's, scope roots the consumer names equal the consumer's, and the producer's own retained inputs belong to the producers they were declared from, as far as the caller's `:records` can tell; an unknown producer refuses (`executor_test/inputs-chain-through-tools-and-stale-scope-or-items-refuse`). Roots nobody names are not checked; the store's full-input key is the ruled home for that. The sphere brush exercises real withheld binding work, provisional history, stale answers and producer subjects through the same executor (`test/app/client/region3d/brush_test.clj`).
