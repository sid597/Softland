...
The client turns text, images, paths and bounded 3D scenes into spatial answers and GPU work. Four content families share an engine for record execution, pure surface paint/sample, coordinates, color, GPU transport and render-target ownership (`engine/executor_test.clj`, `engine/executor_pending_test.clj`, `engine/surface_test.clj`). The browser harness constructs inputs, composes frames and collects evidence.
