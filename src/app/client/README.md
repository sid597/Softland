...
The client turns text, images, paths and bounded 3D scenes into spatial answers and GPU work. Four content families share an engine for record execution, pure surface paint/sample, coordinates, color, GPU transport and render-target ownership (`engine/executor_test.clj`, `engine/surface_test.clj`). The browser harness constructs inputs, composes frames and collects evidence.
...
| [path/](path/README.md) | Records to path values to regions; packing for the camera; CPU membership and coverage; one instanced coverage draw. | Callers execute recipes into path values; renderer systems retain geometry by its full inputs, packs, atlas and instance rows (`path/construction_test.clj`, harness path rates). |
