(ns app.client.substrate.webgpu.region-bindings
...
(def ^:private payload-fields
  [:background :encode-rung :composite])

(defn- changed-payload-fields [old-row new-row]
  (into []
        (filter #(not= (get old-row %) (get new-row %)))
        payload-fields))
...
(defn reconcile-desired!
  "Reconcile prepared, GPU-free desired rows.  Returns the current desired map;
   semantic/binding delta values are drained separately by the renderer."
  [owner rows]
                    ...
                    (fn [next-state region-id]
                      (let [slot (get-in next-state [:slots region-id])
                        ...
                        (cond-> (-> next-state
                                    ...
                                    (update :semantic-deltas conj
                                            (frame-delta/region-topology-delta
                                             region-id :close old-row nil))
                                    (update :binding-deltas conj
                                            (frame-delta/region-lease-delta
                                             region-id (lease-key old-row) nil)))
                          (some? slot)
                    ...
                    (fn [[next-state desired] region-id]
                      (let [input (get rows-by-id region-id)
                            ...
                            row (assoc input :slot slot
                                      ...
                                      :device-epoch epoch)
                            old-shadow? (boolean (:shadow? old-row))
                            new-shadow? (boolean (:shadow? row))
                            topology-delta
                            (cond
                              (contains? opened region-id)
                              (frame-delta/region-topology-delta
                               region-id :open nil row)

                              (not= old-shadow? new-shadow?)
                              (frame-delta/region-topology-delta
                               region-id :shadow-flip old-row row)

                              :else nil)
                            binding-delta
                            (when (not= (lease-key old-row) (lease-key row))
                              (frame-delta/region-lease-delta
                               region-id (lease-key old-row) (lease-key row)))
                            payload-changes (changed-payload-fields old-row row)
                            payload-delta
                            (when (seq payload-changes)
                              (frame-delta/region-payload-delta
                               region-id old-row row payload-changes))]
                        [(cond-> next-state
                           topology-delta
                           (update :semantic-deltas conj topology-delta)
                           binding-delta
                           (update :binding-deltas conj binding-delta)
                           payload-delta
                           (update :binding-deltas conj payload-delta))
                         (assoc desired region-id row)]))
                    [state {}]
           ...
           (fn [state]
             (if (identical? compositor (:compositor state))
               ...
               (let [epoch (inc (:device-epoch state))
                     replacement? (some? (:compositor state))
...
(defn drain-deltas! [owner]
  (let [result (volatile! nil)]
    (swap! (:!state owner)
           (fn [state]
             (vreset! result {:semantic (:semantic-deltas state)
                              :binding (:binding-deltas state)})
             (assoc state :semantic-deltas [] :binding-deltas [])))
    @result))
