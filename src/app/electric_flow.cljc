(ns app.electric-flow
  ...
  (:require contrib.str
           ...
           ; [app.client.playground.actions :refer [context-menu theme-toggle]]
            #_[app.client.utils :refer [ ui-mode edges nodes added-new-nodes? add-dg-new-nodes
                                      is-dragging?  zoom-level last-position subscribe
                                       viewbox  context-menu? reset-global-vals]]
            #?@(:cljs
                [[clojure.string :as str]
                 ;[app.client.utils :refer [!added-new-nodes? !border-drag? !is-dragging? !zoom-level !last-position !viewbox !context-menu?]]
...
(e/defn view []
  (let [current-selection (atom nil)]
    (e/client
      #_(when-not @!added-new-nodes?
          (add-dg-new-nodes.)
          (reset! !added-new-nodes? true))
      ;(theme-toggle.)
      #_(when (some? context-menu?)
      ...
      (svg/svg
        (dom/props {:viewBox (clojure.string/join " " viewbox)
                    ...
                    :style {:min-width "100%"
                            :min-height "100%"
                            :z-index 1
                            :background-color "red"#_(:svg-background (theme. ui-mode))
                            :top 0
                            :left 0}})
        #_(dom/on "contextmenu" (e/fn [e]
                                  (e/client
        ...
        #_(bg/dot-background. (:svg-dots (theme. ui-mode)) viewbox)
