...
(defn initial-sweep!
  ...
  [{:keys [runtime roots on-import]}]
  (let [files (for [root roots
                    ^File f (file-seq (File. (str root)))
                    :when (and (.isFile f) (classify (.toPath f)))]
                f)
...
(defn start-ingest-watchers!
  ...
  [{:keys [runtime roots debounce-ms on-import]}]
  (let [debounce-ms (max default-debounce-ms (or debounce-ms default-debounce-ms))
        ...
        running (atom true)
        run-fn (fn [^File file] (run-import! runtime file on-import))]
    (doseq [root roots]
      (try
                       ...
                       (try
                         (let [^Path rel (cast Path (.context event))
                           ...
                           (cond
                             ...
                             (and (regular-file? child) (classify child))
