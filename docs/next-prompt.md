...
- **seam cuts (the waist, round two)** → `docs/seam-cuts/CONTRACT.md` · `NOW.md` — **CONTRACT CUT 2026-09-02 (cutter Fable 5.1, max); Sid landed the position + both forks same day ("land it · A: data · B: GPU").** Atom 1 = the path seam: stamp, slot, receipts ledger, local color preamble, frame gates, instruments and examples out; grammar declared as data; width on the knot; one hit formula; ops name container ids and the placement tree runs. Atoms 2–4 repeat it on image, text, region; then B (GPU fill: stroker to contours, ear-clip + regime table out); then A (runtime + builtins; the 17 server verb declarations become rows); then the missing floor in canvas order: pointer, in-flight store, wire, clock, culling. PENDING: decisions.md waist amendment (Fable, fresh context, source CONTRACT §0) · falsification round at Sid's hand (§12) · Codex implementation (starter §11).
