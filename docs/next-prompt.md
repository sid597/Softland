...
- **below-the-waist cleanup** → `docs/below-the-waist/waist-cleanup-task.md` · `NOW.md` — **ORIGINAL CUT ACCEPTED 2026-08-21; FOLLOW-ON FACE RECIPE/COMPILER CUT CLOSED 2026-08-24, SID ACCEPTANCE PENDING.** The obsolete browser compiler/builders, shared anatomy material, plain assembly importer, and their minimal projection/import/watch/portal callers are gone with no replacement; durable Rama modules, PStates, stored data, `/mnt/data/rama`, and the verified sibling archive remain. **RENDER-ENGINE JUDGMENT MAP 2026-08-25** → `docs/below-the-waist/engine-waist-map.md` (+ `.html` · `.pdf` twins; Fable 5 max, six Opus gatherers by seam, two independent reviews folded): the engine's *family key* (ten nouns, each kind + pipeline + grammar) is the old world — keep the exhaustiveness fence, change the key to facets, let an entity wear several; convergent cut list contract-ready at Sid's word (the tree · demos · marks vocabulary · the small Blender regardless of the 3D answer · orphan twins · ambient providers · browser globals; position: the arrow family whole); build-stage, not contractable yet (family→facet key · the render packet · a form-free mark road · `region3d_placement`'s input · validator/static sorting · legacy text road); **only Sid: can an entity wear a 3D mesh facet needing depth + spatial queries** — yes keeps the mesh road + scene geometry, no drops the 3D kernel; sequence = rule 3D → land → cut instances → then the key at the renderer's grain.
