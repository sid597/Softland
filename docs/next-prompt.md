...
- **region3d-pointer** → `docs/render-engine/REGION3D-POINTER-CONTRACT.md` + `REGION3D-POINTER-NOW.md` — **CONTRACT CUT 2026-08-11** (Fable, born of Sid driving the seam demo: pointer-anchor drift + W/E/R gizmo confusion). Four structural defects pinned: no pointer anchor in dolly/pan · pan units cross-pair · dual wheel dispatch (region3d capture listener + `scroll.cljs:23` ground road, same node) · gizmo start/move ray scale split + pick≠paint + device-fixed slop. Cure: one typed pointer packet (never cross-pair), anchored dolly/glued pan, one wheel court at `scroll.cljs`, segment-metric gizmo picking + hover. Codex structural round folded (3/3 accepted). Fresh-eyes round RUN 2026-08-11 (Codex at Sid's hand, touch #1): six findings, all folded in-session — ambience reconciliation pinned (§4/F1), S3 projection-custody leg, S4 boundary honesty, S5 court extraction + broadened greps, required hover golden, §13 custody greps; ≥2 decision-changing so the fresh-eyes law stands. NEXT → one implementer session (opening prompt in contract §13, ~28KB boot). Board debt logged in contract §10: `editing_runtime.cljs:287-298` raw-clientX pick path, same defect class, different surface.
