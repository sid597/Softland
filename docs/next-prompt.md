...
- **region3d-pointer** → `docs/render-engine/REGION3D-POINTER-CONTRACT.md` + `REGION3D-POINTER-NOW.md` — **POINTER FUNCTION APPROVED BY SID 2026-08-11; TRANSFORM-LAG CORRECTION BUILT + RECEIPTED; REPAIRED FEEL PENDING SID.** The typed packet, anchored navigation, sole wheel court, complete gizmo pick, and GPU hover are intact. The correction now follows retained-editor architecture: preview/cancel/settle dirty only the transform component, maintain the affected hierarchy and BVH branches, and upload stable-offset instance/glyph/light rows; material/topology changes retain the full oracle road. Actual AMD/DPR≈2 gizmo drag fell from 182–210ms to 11–13ms per frame; 12 moves produced 12 transform updates + 12 instance-row uploads and zero scene derives or mesh uploads. Correction tripwires are 12 tests / 81 assertions green; dev + render-verifier green. NEXT → Sid drives the same gizmo once and accepts/rejects repaired feel. Foreign `family paint is incomplete` and separate `editing_runtime.cljs:287-298` raw-clientX custody remain debt.
