...
- **seam cuts (the waist, round two)** → `docs/seam-cuts/CONTRACT.md` · `NOW.md` — **ATOM 1 PATH SEAM BUILT + CLOSE RECEIPT PASS 2026-09-02 (`9caa2ab`); SID ACCEPTANCE PENDING.** Stamp/slot/ledger/local color/frame-gate/source-fixture forms are gone; path rows travel as validated EDN, width lives on knots, hit and boundary share one formula, ops name container ids, and cid 17 visibly runs through the placement tree. Focused JVM `9/62` green; canonical render replay 6/6 guards green on SwiftShader; two preserved path goldens + the new tree golden match. After acceptance, atoms 2–4 repeat the seam on image, text, region; then B (GPU fill), A (runtime + builtins), and the missing floor in canvas order. PENDING outside this atom: `docs/decisions.md` waist amendment — Fable, fresh context, source CONTRACT §0.
