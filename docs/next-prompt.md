...
- **frame-view / region-binding** → `docs/render-engine/FRAME-VIEW-REGION-BINDING-CONTRACT.md` + `FRAME-VIEW-REGION-BINDING-NOW.md` + `FRAME-VIEW-LOWER-RESOLUTION-CONTRACT.md` + `FRAME-VIEW-LOWER-RESOLUTION-NOW.md` — **LOWER-RESOLUTION CORRECTION BUILT + MECHANICALLY CLOSED 2026-08-11; SID FELT ACCEPTANCE OPEN.** AMD fullscreen now wears current Region3D content instead of checkerboard: desired `3328×2304` → granted `1792×1280` (÷2), refusals 0, zero semantic/effect/plan work, binding update/acquire/retire `1/1/1`; L1–L5, 7/7 goldens, and 19/125 JVM assertions green. Mobile pinch stayed sharp without reaching budget, so that pressure receipt remains pending. Foreign T1 layout and legacy family-paint failures remain board debt. NEXT → Sid gives fullscreen felt accept/reject; no further implementation work is queued. FRAME-RETENTION remains BUILT + RECEIPTED (`FRAME-RETENTION-NOW.md`).
