...
**Vision:** LOG routed through **2026-09-02** (the waist round two, routed: "dead" means wrong form, not uncalled; the ECS layer is his name for above the waist; rulings A verbs are data run by a client runtime with builtins · B fill on the GPU; "first make the existing code how it should be and only then fold the other things in" → atom 1 = the path seam cut, `docs/seam-cuts/CONTRACT.md`. Prior pull 2026-08-28: the phase is deleting; the client is sorted and folded by kind; his 3D line is open; server/ + shared/ get the same sort next.)
