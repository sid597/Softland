...
- **region3d-floor** → `docs/render-engine/REGION3D-FLOOR-CONTRACT.md` — **Package 3 Atom A (the Blender floor), CONTRACT CUT 2026-08-07; falsification round RUN + INGESTED same day** (Codex fresh-eyes, 15 decision-changing findings, all accepted; author repairs landed in place — order token, producer/color-mode/store/pick roads, T2 seam, lease ownership, §5.2/§5.10 schemas, S1–S5 legs; contract now 52KB, round does NOT retire the touch-#1 amendment); NEXT → the implementation session, by CODEX (starter prompt inside the contract, cut for Codex). Atom B (coexistence seam + THE SEAM DEMO) obligations FROZEN in §9; the P2+P3 courtroom defined in §10; durable-custody flip stays Sid's line after it
