...
- **region3d-pointer** → `docs/render-engine/REGION3D-POINTER-CONTRACT.md` + `REGION3D-POINTER-NOW.md` — **BUILT; SOURCE FROZEN; MECHANICAL RECEIPTS GREEN 2026-08-11; SID ACCEPTANCE PENDING.** One typed packet now owns desired-device pointer/view custody; dolly anchors, pan glues from the start camera, one `scroll.cljs` court owns wheel camera dispatch, and segment/polyline gizmo picking has DPR-correct glass slop + GPU hover. Focused S1–S5 are 13 tests / 99 assertions green; protected prior 7 Region3D goldens are byte-unchanged and required `gizmo-hover-x` makes 8/8 deterministic matches. Debt: the broad assertion stopped before Region3D on foreign `family paint is incomplete`; the local product surface did not mount, so lived pointer feel/runtime custody remains RECEIPT PENDING. NEXT → Sid drives `?live-atoms=1&region3d=1&seam-demo=1` at zoom ≠1 and DPR ≥2 across wheel/pan/W/E/R, then accepts or rejects. Separate board debt remains `editing_runtime.cljs:287-298` raw-clientX pick custody.
