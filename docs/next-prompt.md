...
- **seam cuts (the waist, round two)** → `docs/seam-cuts/CONTRACT.md` · `NOW.md` — **ATOM 1 PATH SEAM CLOSED + SID-ACCEPTED 2026-09-02 (`9caa2ab`; Sid: "this session work for row 3 is accepted").** Stamp/slot/ledger/local color/frame-gate/source-fixture forms are gone; path rows travel as validated EDN, width lives on knots, hit and boundary share one formula, ops name container ids, and cid 17 visibly runs through the placement tree. Focused JVM `9/62` green; canonical render replay 6/6 guards green on SwiftShader; two preserved path goldens + the new tree golden match. **CONTRACT-2-4 CLOSED + SID-ACCEPTED 2026-09-02 (`470e11c`; Sid: "both accepted now lets go"). NEXT: the rename, `CONTRACT-2-4.md` §11f, one Codex session, then B (GPU fill).** Image, region and text cleanup is built; image `:writes` is the pool's per-instance GPU-write count, T1's ninth Slug golden is banked, and the final Google SwiftShader replay passes `6/6` guards, `7/7` representative goldens and `7/7` DejaVu goldens over 44 source inputs; `clj -X:test` exited 0 with no emitted totals. After acceptance: B (GPU fill), A (runtime + builtins), and the missing engine in canvas order. The `docs/decisions.md` render-boundary amendment landed 2026-09-02 (source CONTRACT §0).
