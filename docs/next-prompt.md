...
- **below-the-waist census + moves** → `docs/below-the-waist/census-and-moves.md` — **CLOSED 2026-08-19.** Five rows ruled and landed: trail-face retired; legacy editor/dependency residue stripped; invisible renderer nouns stripped; whole pull kept as recovery with explicit transfer duties; T2 joined to one real served durable block through page-local object provenance, correlated decisions, and causally tagged keyed truth. The final member-walk names what is floor and what remains product material. Finish line reached: above-waist UI/UX and product work may fan out concurrently; renderer/WebGPU/verifier/examples stay for later type work. **SUPERSEDED same day, later sitting (Sid): the fan-out line is burned — the waist-cleanup task comes first, all lights out (reads AND writes dark), four-door test over the whole tree; product fan-out begins only when cleanup closes → `docs/below-the-waist/waist-cleanup-task.md`.**
