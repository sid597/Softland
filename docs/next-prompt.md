...
- **below-the-waist cleanup** → `docs/below-the-waist/waist-cleanup-task.md` · `NOW.md` — **CODE CLOSED 2026-08-20; SID ACCEPTANCE + RESTART READBACK PENDING.** All-lights-out cut landed with no migration or duplicate; block-as-atom remains waist, while the current block render channel is explicit one-cut presentation debt. R1–R4 pass; R5 is pending because the fully stopped external cluster did not become conductor-ready on bounded restart.
