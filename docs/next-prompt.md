...
- **region3d-floor** → `docs/render-engine/REGION3D-FLOOR-CONTRACT.md` + `docs/render-engine/REGION3D-FLOOR-NOW.md` — **Package 3 Atom A (the Blender floor), SID-ACCEPTED 2026-08-07 at `?region3d=1` after the bounded camera/gizmo felt repair; CODE `8b1ab98`** (orbit/pan/dolly + W/E/R work as expected; lit/translucent scene felt alive; prior close 23 JVM tests / 296 assertions, 3/3 goldens, S1–S5 green; preserved MSDF counterexample remains foreign RED). LIVE DEBT → object manipulation felt slow; observed/unmeasured/unattributed, one adapter-pinned frame trace at package close before any cause/fix claim. NEXT → **Atom B contract CUT 2026-08-07** (`docs/render-engine/REGION3D-SEAM-CONTRACT.md` + `REGION3D-SEAM-NOW.md`: the coexistence seam + THE SEAM DEMO — placed text/ink, `:region-object` anchoring, one-identity-two-instances, courtroom §9 carries the latency trace); falsification round INGESTED + repaired 2026-08-07 (13/13 findings in place — islands formula verbatim, ref-owner rule + color adapter, `:layout/id` mirror key, one ink-cache cell, executable v2 migrations both families, `:local` required, region door re-homed to `derive-route-set` + region-before-connector order, per-frame atlas custody + Slug refusal, S1–S5 anti-gaming legs); next act = Codex IMPLEMENTS (starter prompt = the contract tail); durable-custody flip stays Sid's line after the courtroom
