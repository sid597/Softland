...
- **frame-view / region-binding** → `docs/render-engine/FRAME-VIEW-REGION-BINDING-CONTRACT.md` + `FRAME-VIEW-REGION-BINDING-NOW.md` — **CONTRACT CUT 2026-08-09** (successor to FRAME-RETENTION §5e/R5 + the Region3D lease-shape rule; summoned by the C1 receipt, `PERF-DOSSIER-2026-08-09.md:212-253`; three review findings folded — touched-set neighbor law, device epoch = system identity, two road values). NEXT → ONE bounded fresh-eyes round (Codex-class, Sid's hand) → author repairs → Sid touch #1 → implementer session (opening prompt at the contract tail). FRAME-RETENTION itself: BUILT + RECEIPTED (twin green on real GPU, n=261; `FRAME-RETENTION-NOW.md`) — no prior board line existed; this line carries both pointers.
