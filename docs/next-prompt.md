...
- **frame-view / region-binding** → `docs/render-engine/FRAME-VIEW-REGION-BINDING-CONTRACT.md` + `FRAME-VIEW-REGION-BINDING-NOW.md` — **BUILT + BLACK-CANVAS REPAIR RECEIPTED 2026-08-10; EXTREME FULLSCREEN ACCEPTANCE OPEN; CODE `ec8c430`.** Lease crossings remain binding-only with zero semantic/effect/plan work. Persistent Region3D and optional blur allocation now preserve the mandatory cached frame reserve, eliminating the whole-canvas `frame/group-output/42` failure; the W4 pool receipt is green. Sid's re-wear reaches the declared Region3D checkerboard refusal at maximum fullscreen workspace zoom, so continuous mesh/text/ink visibility is not accepted there. CONTRACT CUT 2026-08-10 → `FRAME-VIEW-LOWER-RESOLUTION-CONTRACT.md` (the admission rung: budget-aware current-frame lower-resolution road, degrade-before-refuse per the representation ladder; Fable 5, max). ROUND COMPLETE 2026-08-10 (Codex at Sid's hand: 7 findings, 3 decision-changing, all confirmed against source + folded in one batched pass; no recut). NEXT → Sid touch #1, then implement per its §16 opening prompt. FRAME-RETENTION remains BUILT + RECEIPTED (`FRAME-RETENTION-NOW.md`).
