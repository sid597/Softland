...
| the four edits against the checkpoint | first pressure .65 → .4 refused (dab 0 differs); last x 102 → 110 loads; `tool.pickup` .5 → .25 refused; surface 64 × 64 refused | `HANDOVER.md:267`, `wire.js:26-29` | resume with the edited record: `:consumed-items-differ` at 0; complete; `:recipe-differs`; `:recipe-differs` (the surface declaration is a root in the recipe) |
