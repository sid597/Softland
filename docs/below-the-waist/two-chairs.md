...
**The first slice** is the merge of the Claude base into `main`, the three ports, the five fixes and the verifier, landed by Codex directly from the judge receipts and the starter, with no design session before it. Sid, 2026-09-07, after midnight IST: "we do merge first and then the further fixes by codex because as you found out its anal about its boundings and how to do things correct ... i think it would take care of the merge as well so write the full prompt". The design as data for this slice exists already, in the receipts and in the ported code, and the merge, which is Sid's by the house rule, is handed to the landing session at his word, carried in the paste. The push edge and the rest of findings 1 to 4 move to the design that follows. That design is one session, Fable at max, for the pieces both kinds share, the executor and the compositor's paint and sample, with both benches' records as the measure, which may start while the slice lands, since it depends on it in three named places (the executor's landed form, readable in the Codex worktree today; the edit boundary as landed; the callers' names), and amends its file once against the slice's handoff; the 3D host geometry follows in the same file or the next; Codex lands per kind, the path kind's pickup brush and the 3D kind's coating-reading brush through the executor and the compositor, with earlier records kept passing, which is the extension through the existing interfaces that answers whether the waist holds. The production starters are `path-kind/STARTER-production.md` (the landing paste for this slice with Sid's card, the definer's and the judges' pastes named) and `3d/STARTER-production.md` (the shared design session's paste, the 3D kind's landing paste from it, and the card). Sid, 2026-09-06 16:54: "lets fix the docs that needed fixes update our thigns and then we will get to the production in future sessions".
