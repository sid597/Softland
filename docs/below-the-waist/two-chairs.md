...
The judge is the definer's records passing, the harness running and what Sid sees on screen, never one session ranking the other. Sid's card, written for a judge who reads no code and did not choose the approach: every row is something to look at, count, or ask both sessions and compare. Mark each row better, same or worse; do not sum; the roles come from the pattern. Each run keeps its model, version, effort and tools attached, since the result is about those setups on this task.

| Look at, or ask | Good looks like | Bad looks like |
|---|---|---|
| **The app runs the harness fixtures** (the pen stroke, the draw tool's Z, the border, pressure ink, the holed shape), with the bench open beside it | the same picture as the bench: the crossing painted once, the tapers swept, the border one pixel at every zoom | anything missing, doubled or greyed; "it runs" without the screen |
| **The definer's records, run by the session, numbers printed** (the crossing .62 as a union and .8556 as dabs; the pickup texel (0.013, 0, 0.987, 1); the proofs red after blue) | the bench's numbers, from `bench-9/HANDOVER.md`'s tables, reproduced; a mismatch reported as a finding with a cause | numbers not run, or a mismatch explained away |
| **Ask: what did you delete?** and look at `git diff --stat` on its branch | the old triangle shaders, mesh preparation, zoom bands and the tessellator's use by placed ink are gone; deletions in `path/` and `region3d/on_plane` | a build that only adds; the old route still alive beside the new |
| **Placed ink in 3D still draws** on screen | the on-plane consumer moved and shows | 3D ink broken or untouched |
| **Ask: draw me the data flow in ten lines**: where a record enters, what is derived, who owns each piece of state, what triggers work, where reactive code is and why | Sid can hold it after one reading; every state has one named owner; reactive used where a reason is given, or honestly "nowhere" | a list of namespaces; owners unnamed; Missionary everywhere or nowhere without a reason |
| **Ask: what did you carry forward from the old tree, and why each?** | few things, each argued from the experience under Sid's ruling; the filler's algorithm kept because it is the best coverage, not because it was there | "the text renderer's coverage, it existed"; or everything rewritten with no reason either |
| **Ask: what broke while you built, and how did you find it?** | real failures with receipts (a screenshot, a number, a failing test), found by running, then fixed | smooth sailing; failures found only by Sid |
| **Ask: show me the test that fails if the crossing were painted twice**, and run it | a named test, run in front of Sid, red when the rule is broken | no such test; a test that cannot fail |
| **Open the README of the new namespace and read for two minutes** | Sid knows what is implemented and how, in plain words, and it matches the code's date | corpus words, a plan instead of a description, or a README older than the code |
| **Ask: which decision are you least sure of?** | one or two named calls with what would change them | "none", or a list of everything |
| **Cost and intervention**: tokens and wall time per session; how many times Sid had to redirect; how many questions it asked Sid | fewer of each, with the questions being real forks | a cheap session that needed steering, or an expensive one that asked nothing and guessed |

Stage two, the exchange, is judged the same way: things landed on the other's branch (a bug with a receipt, a test, a repair), and whether the other's sound decisions were kept or rewritten for taste.
