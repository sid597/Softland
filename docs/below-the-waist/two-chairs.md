...
**After phase B**: Sid's rulings on the asked positions, then boots on the ground for the pieces the attacks stopped moving, behind the attack records as tests; the definer moves to the next kind under the same frame, where attacks can reframe again. The bench keeps taking attacks on the pieces still moving.
