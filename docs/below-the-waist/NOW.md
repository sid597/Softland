...
Status: **CONDUCTOR CUT BUILT + RECEIPTED 2026-08-28 (`c13711c` moves · `bff327e` cut); SID ACCEPTANCE PENDING.** Conductor cut (scene tape, frame graph, delta spine, maintain zone) — Sid's ruling 2026-08-28; painters kept as libraries with no caller; no replacement built. Receipt: JVM 30 tests / 208 assertions / 0 / 0 (runner floor 326/4379 = old floor minus what left); CLJS 83 files / 0 warnings; six guards + 7 DejaVu Slug, Ubuntu mixed-face, chrome, path goldens green; Region3D floor (PBR + shadow oracle, S5 lifecycle, worn leg) green through a verifier-side direct driver; S1/S3/S4 greps clean; `git diff --check` clean; custody: no server/shared/resources/archive file touched. The original waist, instance, and text-road cuts remain accepted; §2 and §3 were not opened.
...
Protected: shared shaping/layout/metrics, TTFs, HarfBuzz, ray/plane and anchor projection, path/text hits, `pick-region`, shadows, Region3D composite, and archive/data custody remain.
