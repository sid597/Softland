...
Softland · client · path kind · 2026-09-06 · sessions 11, 10, 9, 8 and 7 folded, the definer's attacks 1, 2 and 3 folded, Sid's two late questions worked
...
beside the value, never inside it (the definer, attack 2)
  ...
  checkpoint?        { at }   a construction continuation: the state after `at` items
                     (surfaces as content with map and colour, carried values, the next
                     item) saved with what it depends on; resumed, never re-run
...
to and from the compositor (surfaces)
  paint(surface, region, paint, clips, blend) → surface   a new value with a key and a parent;
                                                          ...
                                                          is reused only after a last read
...
---
Landed under `docs/below-the-waist/path-kind/`: this page and its `.md` twin, session 8's fact base with the hunter reports and the six closing folds, session 7's brief, page and three hunt reports under `sources-7/`, session 4's bench and its handover under `bench-4/`, session 9's bench under `bench-9/` with session 10's additions and their handover, the fact bases `fact-base-9.md`, `fact-base-10.md` and `fact-base-11.md`, and the definer's `attack-1.md`, `attack-2.md` and `attack-3.md`. Positions are marked and the verdicts are Sid's.
