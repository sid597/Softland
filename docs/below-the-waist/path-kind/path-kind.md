...
Softland · client · path kind · 2026-09-06 · sessions 10, 9, 8 and 7 folded, the definer's attacks 1 and 2 folded, Sid's two late questions worked
...
- The executor is a vocabulary and an order, not one interpreter. It moves values between capabilities and keeps the sequence a construction declares; every heavy thing happens inside a capability, per pixel on the GPU or per point in native code, and instances are many. The one thing that serializes is a surface read before it is written, and that is the compositor's, bounded to brushes that read.
...
to and from the compositor (surfaces)
  paint(surface, region, paint, clips, blend) → surface   a new revision
...
| per brush step | a painting surface's content: each paint is a new revision; a committed dab is never re-applied by a frame; a pressure edit on an earlier dab replays from the start or a checkpoint (attack 2) | the compositor's retained surface, at its declared resolution; the CPU twin composites the same way | nothing: no brush, no surface, no read of a target anywhere |
...
| Authored sources, logical results and execution resources kept distinct; materialize what the next operation needs | Position D: pixels, or a replayable history; the framebuffer handle is replaceable | "surfaces are values" in figure 1; the bench's CPU host copies on paint and its GPU host paints in place, and the handover names where they would differ |
...
- **Surface reads serialize; everything else batches.** Plain accumulation is one instanced draw in painter's order, which the API keeps. A brush that reads what it painted needs the previous dab resolved first: on a fragment road a copy under the dab per dab, on a compute road sequential dabs inside the tile (Krita: tiles and threads on the CPU; FIELD). That, not the executor, is where a painting brush's throughput lives; bench 9 counts 24 syncs for 24 dabs.
...
---
Landed under `docs/below-the-waist/path-kind/`: this page and its `.md` twin, session 8's fact base with the hunter reports and the six closing folds, session 7's brief, page and three hunt reports under `sources-7/`, session 4's bench and its handover under `bench-4/`, session 9's bench under `bench-9/` with session 10's additions and their handover, the fact bases `fact-base-9.md` and `fact-base-10.md`, and the definer's `attack-1.md` and `attack-2.md`. Positions are marked and the verdicts are Sid's.
