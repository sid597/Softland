...
| session 1 (`3d-kind.md`, `bench-0/HANDOVER.md`) | the rate row "on its own inputs" (the axis table, story 13); render as a request naming its quantity (the Out block, Sid's question 7); copies are two things; face identity does not come free; event routing (the field list); the three camera operations with its bench's measurement (Sid's question 5); the filler callable with an anisotropic footprint (the path-kind list) | its "page zoom is focal length" was corrected by its own reviews to magnification, which is what this page said; its "a space is a frame plus an order rule" is not adopted: this page keeps planar and spatial as the two kinds, because the walker and the packer branch on more than the order rule (depth, units, a metric) |
