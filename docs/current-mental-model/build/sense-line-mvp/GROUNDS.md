...
- **Placement RULED: Option A** (Sid, 2026-07-09): "we will go with the recommended … this is what it was designed for" — block layer = `sense-block-v0` distiller adapter + driver over the container kernel + relation kernel; CONTRACT v2 in place. Corner-checks live as contract stop-clauses (strata import semantics · anchor determinism · derived-text copy). Same message: no-banners workflow rule (direct replacement, git is history); code-size audit queued; read-grounded MAP.md started.
