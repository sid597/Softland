...
**Journal grain — semantic, not transport (Sid's catch, 07-15: "is this semantically correct? how would undo work? does it not bloat the log?"):** three grains, kept distinct.
1. *Pipe grain* — block-write streams per-op (≈ per keystroke, "BW-T3 grain") for the 7.66ms echo feel. Transport frames, RAM-only, **never enter the journal**.
2. *History grain* — the journal records **settled events**: a text-edit burst coalesced when it settles (~1.5s pause, leaving the block, or any signing event — thresholds tunable), carrying the resulting text; plus every signing-grade event (wish, accept, revert, run-agent). Containment ladder: keystroke ⊂ burst ⊂ block-change ⊂ turn ⊂ episode — the permanent record enters at *burst*.
3. *Flush policy on the journal*: settled text events ride group commit (~50ms); signing events flush immediately, durable-before-side-effect (an agent must never have run for a reason the land doesn't remember).

**Undo law:** the undo unit IS the burst (people undo intentions, not letters); Ctrl-Z appends a compensating event — history only grows, nothing is erased. **Bloat:** burst grain ≈ hundreds of events/day, tens of MB/year, replay in seconds; keystrokes never reach the permanent record (also the kinder answer to the tentative-thought membrane — the land keeps what you wrote, not the tape of how your hands shook). **Crash loss bound, restated honestly:** the in-flight burst — a few seconds of typing, any editor's unsaved-moment bound; an optional scratch tier (ring file at op grain, recovery-only, never history) exists if that ever stings.

**Falsification list for the slice contract:** replay must be pure materialization — executors/LLM intents must NOT re-fire on replayed events (intent-vs-executor split verified under replay); every depot entry point passes the shim (foreign appends audited); group-commit + signing-flush latency measured on the real write path (block-write p95 must survive; signing events verified durable-before-side-effect); truncated-tail recovery tested by killing the JVM mid-write; replay idempotence tested by double-replay; **replay-path state must equal op-path state** (type real text, snapshot PStates, reboot, replay, diff — burst coalescing must lose nothing the views need); boot time at N months of events measured, snapshot threshold named.
