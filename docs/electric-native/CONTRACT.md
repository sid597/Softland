...
- **No Electric in `deps.edn`.** Courier = SSE + Missionary. → the transfer bench (PROBLEM-SPACE probe 1) decides; the courier is one file each side (§9.1).
...
**9.1 Courier — SSE + Missionary now; Electric's keyed machinery at the bench.** Content (§4) is courier-agnostic; the courier is `host/courier.cljs` + the stream arm's writer — one file each side. Replacement trigger, named so this is a stage not a patch: the **transfer bench** (PROBLEM-SPACE probe 1 — four algebras × `e/diff-by` losslessness × two-edits-one-tick), run when the Block probe builds the face both ways. Counter-position (lawful, not taken): re-add Electric now and ride `e/diff-by` from day one — pays the dependency earlier, gets reactive consistency for free; its cost is L16's producer-side comparison-minting. Risk of the taken position: an owned lane entrenches; the fence is the frozen format + one-file courier. One Jetty thread per lens (blocking `StreamableResponseBody`) is the courier's known ceiling; it is fine for the felt pass and named here.

**9.2 The host (composition) — the Block probe decides.** Missionary host here; Electric generic host gets its fair test on the Block face built both ways (road 4). Nothing in this package forecloses it: faces are data, the store is the target.
