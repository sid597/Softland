...
1. **Text runs in the parallel wave, not after it.** After the render test harness split every kind edits only its own files (§4). Text's cleanup touches `text/*` and `harness/text.cljs` and nothing the others touch. Its dirty check is a non-goal here, not deferred: the layout key (`text/layout.cljc:317`) and the layout / paint / GPU-geometry partition (`layout-cache-acquire`, `layout-cache-report`, `:1119-1179`) that `docs/shaping-correction/CONTRACT.md` §6 and §7.3 contracted are in the tree, tested by two namespaces, and called by nothing in `src/` since their consumers left at the waist cut. Wiring them into the entry point is that package's next act, under its own ownership of `layout.cljc` and `renderer.cljs`.
2. **The CPU color law has one home because oracles and diagnostics must reproduce the shader, not because renderers should convert on the CPU.** Region is a lit world in a linear working space: its five uniform and instance colors (`region3d/renderer.cljs:502-508`, sites `:551 :552 :582 :621 :1065`) and the on-plane ink's per-vertex color (`on_plane.cljc:119-131`; one caller, `on_plane_renderer.cljs:200-202`, landing at `@location(5)` untouched) are linearized unconditionally, mode or no mode, and the compositor encodes at present (`engine/compositor.cljs:38`). Neither region shader converts: zero `srgb` and zero `device/` in both renderers. So region's two copies stay CPU-side and call one function; the JVM oracle (`test/app/client/region3d/oracle.cljc:31-33`) and the render test harness's diagnostic (`harness/core.cljs:759-766`) are the other two consumers. `engine/color.cljc` holds resource maps and no conversion today (`:1-48`); it gains the pair. Image's copy is WGSL (`image/renderer.cljs:84-87`) and folds into `device/scene-color-wgsl` with one exact call shape (§4b).
3. **The split needs a sixth file.** `core.cljs` requires the kinds for dispatch; the kinds need the shared helpers; a kind requiring core is a cycle. The render test harness's shared helpers (`:33-107`; `zoom-cases :39`; the q8 transport `:176-206`; `:1537-1576`; `w4-read-texture! :1646`; and five pixel helpers the image and text layers lend to path and region: `boundary-pixels :207`, `byte-delta :703`, `pixel-rgba :711`, `srgb->linear :759`, `linear->srgb-byte :764`) becomes `harness/shared.cljs`; core keeps dispatch and `start!`. The text section's GPU setup lives inside `run-harness!` (`:2789-2818`) and is lifted into `run-text-slug!`; `load-t1-provider` (`:1577`) has zero callers and goes.
4. **Image's drawn thing has no revision and its revisioned thing is never drawn.** The draw item the renderer takes is `{:id :x :y :w :h :image/digest :image/uv :image/opacity :image/tint :container-idx}` (`harness/core.cljs:513-518`); the component row with `:image/revision` reaches only `component-cache-key` and a test fixture (`image/component.cljc:103,145`; `component_test.clj:22`). A dirty check on the row's revision needs the row to be what is drawn: the row absorbs rect, crop and paint, and the draw item becomes `{:image/material row :container cid}` (§5, F1).

What held: the buffer index resolver moves into the engine (path holds the only `buffer-index`; `on_plane.cljc:153` already resolves `(:container region-draw-item)` through an world-transform map, the right form next entry point); the manifest is completed, as a glob rather than a longer hand list (it drifted by four files); image and region build in parallel; the shared step goes first because it is what makes the rest parallel.
...
| piece | verdict | why |
|---|---|---|
| `buffer-index` in `path/frame.cljc:16-22` | move to `engine/transform.cljc` | three kinds need it; a GPU buffer index is resolved by the tree, once |
| `placement.cljc:29` private `finite-number?` | replace with `schema/finite-number?` | routed by CONTRACT.md §1 |
| `validate-affine` (`placement.cljc:33-39`), `spec->affine`'s open destructure (`:41-57`), `add-group`'s three throws (`:82-104`) | replace in one move | the one placement schema across kinds (routed here by CONTRACT.md §2): a group spec is declared data checked by the engine; the registry's rejections are named. Two callers in the tree (`harness/core.cljs:1482`, `frame_test.clj:34`), both pass an affine. |
...
| `engine/schema.cljc`: no rule for a map keyed by ids; no data on a form rejection | add `[:map-of key-pred spec]` and `:explain` | region's scene is `object-id → object`; its cycle rejection must name the object |
| `harness/core.cljs`: 2926 lines, four sections, one file | split by kind: `shared`, `image`, `path`, `region`, `text`, `core` | one file, three sessions is the corpse in memory `feedback-parallel-sessions-shared-branch-git`, twice |
| `harness/core.cljs:1577` `load-t1-provider` | remove | zero callers |
...
| piece | verdict | why |
|---|---|---|
| `:image/ingress-receipt` on the source row, the `ingress-receipt` constant, the equality check (`material.cljc:19-27, 44, 64-68`) | remove | the row carries a copy of the reader's own constant: equal or rejected, it says nothing. The decode chain is the engine's. No test asserts the rejection (`component_test.clj:53-60`). |
| the stats ledger: `:!receipt`, `receipt-row-counts`, `publish-image-receipt!`, `record-image-receipt!`, the `__softland_image_ingress_receipt` global (`renderer.cljs:283-306, 448-450, 562, 580, 617-637, 665-677, 705-708`); `image-ingress-receipt` in the render test harness | replace | the global has zero readers; the ledger's readers are six render test harness sites. One renderer read is real: the draw item section consults it so a rejected digest keeps its status when its placeholder paints (`:665-677`). Status becomes a value in the residency map; rejections and rebuilds become return values (§4b). |
| `:container-idx` on the draw item (`verifier:518` → `renderer:141` → `material.cljc:336`) | replace | a literal 0 copied into word 13, never through the tree. The draw item names `:container`; vertex packing resolves through `transform/buffer-index`. |
| the prepare-key dirty check `{:images images :resources @!resources}` under `=` (`renderer.cljs:695-698`); `:!last-prepare-key`; `:!last-images` (reset, never dereferenced) | replace | a deep compare of every draw item and every resident texture, every frame; nothing on the drawn thing carries a revision. One key: `[material-id revision container]` per draw item plus the residency revision. |
| `:!shape-rev`, `:frame-input/identity` (`renderer.cljs:445`) | remove | seeded, never read; cargo from other kinds |
...
| `validate-source!`, `validate-component!`, `material-required-keys`, `material-optional-keys`, `legal-*` as throw guards (`material.cljc:16-17, 38-69, 100-135`) | replace in one move | twelve checks, every one expressible as declared schema; the source schema becomes fail-closed (a widening; every in-tree source row carries exactly the checked keys, `verifier:490-500`) |
...
Kept, right form: `sha256-digest?`, `register-verified-source` (the digest proof), `empty-source-registry`, `resolve-source`, `source-cache-key`, `component-cache-key`, `normalize-crop`, `classify-quad`, `half-open-hit?`, `clip-placement`, `crop->uv`, `mip-level-count`, `mip-sizes`, `texture-bytes`, `atlas-config`, `placement-tier`, `empty-atlas`, `atlas-place`, `placement-plan`, `instance-words` (takes the resolved buffer index; fat, not wrong), `contiguous-binding-runs`, the `-srgb` texture view (`renderer:265-281`), `:!source-bytes`, `:!source-registry`, `:!atlas`, `:!resources`, the mip system, the device-loss rebuild as a returning function.

### 1c. Region

| piece | verdict | why |
|---|---|---|
| `:container-idx` on the draw item (`renderer.cljs:972`) into the composite row (`:739, :745`) | replace | a buffer index memcpy'd into the composite buffer. The right form is next entry point: `on_plane.cljc:153` resolves `(:container region-draw-item)` through the world-transform map. The draw item names `:container`; `upload-composites!` resolves through `transform/buffer-index`. |
| the per-region deep `=` dirty check (`renderer.cljs:976-987`) | replace | `row` holds the whole derived scene: `:maintained` with `:instances`, `:triangles-by-object`, `:bvh` (`scene.cljc:641-653`), plus `:draw-item`, `:gpu`, `:placements`, `:shadow-space`. `=` walks the geometry per region per frame. One key per region: `[id revision container]` plus `[zoom dpr session-revision]`. |
...
| `:!receipt` in the renderer (`:456, :999, :1146, :1157`), `region3d-receipt` (`:1202`), on-plane's `:!receipt` (`on_plane_renderer.cljs:122, 286-294, 318`), `placement-receipt` (`:321`) | replace | ledgers read only by the render test harness (`core.cljs:1828, 1987, 1998, 2483-2485, 2525`); the per-frame return of `prepare-region3d-frame!` carries that frame's counts; the render test harness accumulates |
...
| the open extension (`material.cljc:2-4` "Unknown fields are kept"; `component_test.clj:81-83` pins `:future/shape :preserved`) | close | the row boundary's structural tier: "a schema that is data — no unknown key rides" (`docs/decisions.md`, the render boundary). The test flips to a named rejection. F2. |
| region's `finite-number?` (`:80`; ten internal uses, zero external) | remove | `schema/finite-number?`, identical |
| `legal-display-modes` (`:20`, zero uses); `legal-camera-kinds` (`:19`, error data only) | remove | dead; the lens `case` discriminates |
| `tagged-linear` (`renderer.cljs:502-508`), `srgb-channel->linear` (`on_plane.cljc:119-123`) | replace | `color/srgb-channel->linear`, the one home; byte-identical constants |
...
| piece | verdict | why |
|---|---|---|
| `:container-idx` read on the style (`glyph_pack.cljs:138-140`) and the text map (`renderer.cljs:605`); no writer anywhere in `text/*` | replace | the draw item template carries `:container`; `pack-draw-item!` resolves through `transform/buffer-index`; `update-text-data` takes the world-transform map. Fail closed: a draw item without `:container` is rejected (F8). |
| `:!shape-rev` (`renderer.cljs:361, 437, 755-757`), `:frame-input/identity` (`:360, :436`) | remove | written, never read |
...
| `!text-layout-fallbacks` global atom and `text-layout-fallback-report` (`renderer.cljs:490-496, 517`) | replace | a process-global diagnostic in `src/`, read by the render test harness's F3 consistency check (`core.cljs:2672-2693`); the fallback count is a fact of the call, returned under `:fallbacks` |
...
Kept: `text-clip-runs`, `contiguous-state-runs` (`renderer.cljs:764-793`; uncalled; partition by `[:clip :container]`, right form once `:container` is an id), `layout-key` and the layout cache (shaping-correction's, untouched), `:layout/id`, everything the shared color section already does (`renderer.cljs:89, 311, 340`).
...
- Source structure: kind → engine, never engine → kind, never kind → kind except `region3d/on-plane` (CLAUDE.md). `harness/*` is the driver, not a kind: it may require any kind, and `harness/region.cljs` requires `harness/path.cljs` for its fixture builders (`path-draw-item`, `path-ink-material`, `path-polygon-material`; today `core.cljs:1770, 2434-2447`). Fails if any `image/*`, `text/*`, or `region3d/*` file other than on-plane requires another kind.
...
**`src/app/client/engine/transform.cljc`**
- `buffer-index [world-transforms container] → int`, or throws `{:error-type :transform/unknown-group :container c}`; moved from `path/frame.cljc:16-22` verbatim but for the error type.
...
- `add-group` rejects by name: `:placement/reserved-cid`, `:placement/duplicate-container`, `:placement/parent-missing` (today's three throws at `:90-96`, as data). Requires `app.client.engine.schema`.

**`src/app/client/engine/schema.cljc`**
- `[:map-of key-pred spec-or-pred]`: the value must be a map (`:schema/map-required`); every key passes `key-pred` (`:schema/invalid-key`, path `(conj path k)`); every value is checked as a vector item is, `check-form!` for a spec map or `check-predicate!` for a predicate, at `(conj path k)`. About 15 lines beside `check-vector!`.
...
- `tagged`: `{:keys #{:rgba :color-space :alpha-association} :validators {:rgba schema/valid-rgba? :color-space #{:srgb} :alpha-association #{:straight}}}`. Requires `app.client.engine.schema`.

**`src/app/client/path/frame.cljc`**: `buffer-index` gone; `frame-key` unchanged. **`src/app/client/path/renderer.cljs:139`**: `(transform/buffer-index world-transforms (:container draw-item))`. **`test/app/client/path/frame_test.clj:36`**: the buffer index law moves to `test/app/client/engine/transform_test.clj` with the new error type; path's S1 regression test in `harness/path.cljs` expects `:transform/unknown-group`.

**The render test harness split** (`src/app/client/harness/`), by the layers measured at `8dad07c`:
- `shared.cljs` (new, about 12KB): `:33-107` (`canvas-size`, `color-format`, `promise-mapv`, `bytes->hex`, `sha256-*`, `opaque-png-data-url`) · `zoom-cases :39` · `q8-world-transforms`, `run-q8-transport!` `:176-206` · `:1537-1576` (`selected-limits`, `adapter-information`, `shader-digests`) · `w4-read-texture! :1646` · the promoted `boundary-pixels :207`, `byte-delta :703`, `pixel-rgba :711`; `srgb->linear :759` and `linear->srgb-byte :764` become one-line calls to `color/*`.
- `image.cljs` (about 30KB): `:479-1096`, entry `run-image-step!`.
- `path.cljs` (about 19KB): `:1097-1536`, entry `run-path-step!`.
- `region.cljs` (about 45KB): `:1681-2547`, entry `run-region3d-floor!`; requires `verifier.path` for `path-draw-item`, `path-ink-material`, `path-polygon-material`.
- `text.cljs` (about 26KB): `:108-475` minus the q8 pair; `:1589-1645` (`t1-layout-receipt`; `load-t1-provider` dropped); `:2549-2747` (the flat section); plus `run-text-slug!` lifted verbatim from `run-harness!` `:2789-2818` (both text systems, `decode-glyph-curves`, the harness, `run-case!` over the cases, `run-ubuntu-mixed-case!`). `image-record :389` is the slug section's record and moves here.
- `core.cljs` (about 11KB after): the `ns`; `run-harness!` calling `text/run-text-slug!`, `image/run-image-step!`, `path/run-path-step!`, `region/run-region3d-floor!`, `text/t1-layout-receipt`, `text/run-text-flat-route!` and assembling the same result map with the same keys; `run-region3d-floor-harness!`; `start!`. `shadow-cljs.edn` untouched: `app.client.harness.core/start!` remains the entry.
...
**Tests:** new `test/app/client/engine/color_test.clj` (S0) and `test/app/client/engine/schema_test.clj` (S0), both registered in `test/app/test_runner.clj` `pure-namespaces`.

### 4b. Image (after the shared step)

**`src/app/client/image/component.cljc`** (after: about 300 lines)
- `source`: the declared source schema (§6). `validate-source! [s]` = `(schema/check source s)`.
- `schema`: the declared row schema (§6). `validate-component! [m]` = `(schema/check schema m)`.
- `canonical-component`, `component-cache-key`: take a validated map, never re-check.
- `instance-words`: takes `{:rect :uv :tint :opacity :buffer-index}`; the buffer index arrives resolved.
...
- `prepare-image-frame! [system draw-items world-transforms] → {:changed? bool :writes n :instances n}`; `:writes` is the pool's per-instance write count as `batch-update-pool!` returns it, one meter, never reshaped to a contract number (repair ruled 2026-09-02: the first build reported `:writes 1` beside a measured `:item-writes 2`): `key = (frame/frame-key draw-items @!residency-rev)`; equal to `@!last-frame-key` → `{:changed? false :writes 0 …}`; else resolve each draw item through the residency map and `(transform/buffer-index world-transforms (:container draw-item))`, `batch-update-pool!`, store the key. A rejected or unavailable digest resolves to the placeholder binding with its status untouched.
- `draw-image-frame!`, `destroy-image-system!` unchanged in meaning.

**`src/app/client/harness/image.cljs`**
...
- `image-draw-item` → `{:image/material row :container cid}`.
- the section builds a registry, `transform/world-transforms`, `device/write-groups!`, and passes `world-transforms` to `prepare-image-frame!`.
- the three ledger-shaped lifecycle assertions (`:970-977`, `:1006-1013` at `8dad07c` numbering) read the rebuild's return; `run-color-evidence!` keeps its keys; `image-ingress-receipt` gone.
- I1's golden added, registered in the manifest's `imageAtomCases`.

**Tests:** `test/app/client/image/component_test.clj` rewritten to §7 (the `:image/ingress-receipt` fixture line goes); new `test/app/client/image/frame_test.clj` (I3's key law); `test_runner.clj` gains `app.client.image.frame-test`, one line, committed alone.

### 4c. Region (after the shared step)

**`src/app/client/region3d/component.cljc`** (after: about 380 lines)
...
- `schema`: the declared region schema (§6). `validate-region! [r]` = `(schema/check schema (canonical-region r))`; returns the canonical map.
...
- `acyclic?`, `parents-exist?`, `ids-match-keys?` with `:explain` naming the `:object/id` and `:cycle-at`; discriminated unions by the `geometry-matches-kind?` idiom (`path/component.cljc:70-76`).
...
**New: `src/app/client/region3d/frame.cljc`**: `region-key [draw-item zoom dpr session-revision] → [id revision container zoom dpr session-revision]`; `frame-key [draw-items zoom dpr session-revision]`. About 15 lines, JVM-tested.

**`src/app/client/region3d/renderer.cljs`** (after: about 1120 lines)
- `tagged-linear` (`:502-508`) → `color/srgb-channel->linear` at its five sites.
- `prepare-region3d-frame! [system {:keys [regions]} session opts]`: `opts` gains `:world-transforms`, required. Per draw item, `key = (frame/region-key draw-item zoom dpr (:revision session-layout-snapshot))` (the snapshot's `:revision` is read at `on_plane.cljc:44` today); equal to the prepared row's key → the row is reused `identical?`; else the existing `scene/evaluate-scene` section runs from the prior `:maintained` (a full derive on a new region, affected-id maintenance on a changed one). Returns `{:regions {id {:changed? bool :full-rebuilds n :instance-uploads n :bvh-refits n :region-encodes n}} :composite-uploads n :held-passes n}`: that frame's counts, never accumulated.
- `upload-composites!` and `composite-row-bytes` take `:buffer-index`, resolved as `(transform/buffer-index world-transforms (:container draw-item))`.
...
**`src/app/client/region3d/on_plane_renderer.cljs`**: `:!receipt`, `placement-receipt` gone; `prepare-placements!` (`:238`) already returns `:packs`, `:changed?`, `:placements`, `:census` and adds `:uploads`, `:ink-vertices`, `:over-limit` to that return instead of the swap at `:286-294`; `draw-placements!` (`:305`) returns its draw count instead of the swap at `:318`.

**`src/app/client/harness/region.cljs`**
- fixtures write `:region/revision` as the content hash; `region3d-draw-item` → `{:region/material row :container cid :region3d/resolved-placements …}` with the composite rect on the row as `:region/rect`.
- the section's registry, `world-transforms` and `write-groups!` already exist for the surround path system (`:2431-2447` at `8dad07c`); the same `world-transforms` goes to `prepare-region3d-frame!`.
...
**Tests:** `component_test.clj` rewritten to §7 (the `:future/shape` case flips to `:schema/unknown-key`; the cycle case matches `:region/parent-cycle` and reads `:cycle-at` from `ex-data`); `oracle.cljc:31-33` calls `color/srgb-channel->linear`; new `test/app/client/region3d/frame_test.clj` (R3's key law); `scene_test.clj`, `on_plane_test.clj` pass unchanged but for the validated-row entry; `test_runner.clj` gains `app.client.region3d.frame-test`, one line, committed alone.

### 4d. Text (after the shared step; ownership in the starter)

**`src/app/client/text/layout.cljc`**: `line-paint-draw-items` (`:1245-1261`) carries `:container` from the template onto every draw item; `legal-zoom?` public (`:1046`), the throw unchanged; `:input-hash` gone from `:receipts` (`:303, :1082`).
**`src/app/client/text/layout_oracle.cljc`**: `:535-539` calls `layout/legal-zoom?`; `:578` `:input-hash` gone.
**`src/app/client/text/glyph_pack.cljs`**: `pack-draw-item!` takes the buffer index from the draw item, `(transform/buffer-index world-transforms (:container draw-item))`, resolved once per draw item before the glyph loop; the `:container-idx` read gone.
**`src/app/client/text/renderer.cljs`**: `update-text-data` takes `:world-transforms` in its opts and threads it through `pack-instances-flat` to `pack-draw-item!`; `:605` reads `(:container txt)`; `:!shape-rev`, `:frame-input/identity`, `:span-receipt`, `!text-layout-fallbacks`, `text-layout-fallback-report` gone; `shape-text` and `pack-instances-flat` return `:fallbacks {…}` with today's keys (`:490-496`).
**`src/app/client/harness/text.cljs`**: templates and hand-written draw items carry `:container 0` (`:456-457, 2662, 2668-2670, 2717` at `8dad07c`); both text sections build a registry and pass `world-transforms`; the F3 consistency check reads `:fallbacks` from the return; T1 returns its tree evidence (the deterministic hash and the placement check). Its golden PNG and runner row are NOT text's: `run_verifier.mjs:206-210` hardcodes the golden list and `:302` asserts exactly one Ubuntu golden, so registering a ninth is a runner edit; the last closer registers it at package close, one bounded block beside F7's (ruling, Fable, 2026-09-02, on text's fork in `NOW-2-4.md`).
**Tests:** `test/app/client/text/flat_route_test.clj` updated for `:container` and the returned fallbacks; no new namespace.
...
**S0 — the shared step changes no byte.** `npm run verify:render-engine` PASS with every golden in `test/app/fixtures/render_engine/gpu-goldens/manifest.json` and every evidence equal to the path close's run (NOW.md: holed `645f9536…410b`, translucent `5bf612e2…ea79`, tree `ca96286b…53ba`; all 33 PNGs by manifest); `shaderDigests` unchanged; the evidence's `sourceInputs` count is the glob's. JVM: the path suite green with `:transform/unknown-group`; `placement_test`: a five-number affine, a spec with `:tilt`, `:camera :orbit`, and a parent that does not exist are each rejected by name, and the registry the render test harness's path section builds (`core.cljs:1482`) composes to the same `world-transforms` map; `color_test`: the four breakpoints (0, 0.04045, 0.0031308, 1) exact, `0.5 → 0.21404114048223255` within 1e-15, encode∘decode identity within 1e-12 at 0.0, 0.25, 0.5, 0.75, 1.0; `schema_test`: `[:map-of keyword? spec]` rejects a string key as `:schema/invalid-key` at `[:scene "x"]`, checks each value at its path, and a failing form validator's `:explain` map rides in `ex-data`. Wrong build that passes a weaker test: a split that leaves `run-text-slug!`'s systems inside `run-harness!`; §10's ownership check catches it when the text session must edit core.

**I1 — the hierarchy runs.** Render test harness golden. A registry with group 17 under the root at `[0.5 0 0 0.5 40 20]`; one image row drawn with `:container 0` and again with `:container 17`; `world-transforms` → `write-groups!` → `prepare-image-frame!`. The golden shows the second quad at half size offset by (40, 20). A draw item naming a group the hierarchy lacks throws `:transform/unknown-group` before any write. Wrong build that passes a weaker test: the buffer index taken as the cid; 17 gets buffer index 1.

**I2 — rows travel and the schema rejects by name.** JVM. `(edn/read-string (pr-str row))` of each fixture validates, has `nil` metadata, and gives the same `component-cache-key` and the same 13 instance words. Rejections, each by its named type: an unknown top-level key; `:image/extensions`; a source carrying `:image/ingress-receipt`; a source missing `:image/bytes-route`; a tint of five numbers; a rect with `:h 0`; an opacity of 1.5; a crop given as a vector. Wrong build that passes a weaker test: `validate-source!` left open-world; the extra-key rejection catches it.

**I3 — the dirty check is the revision plus residency.** JVM for the key, render test harness for the writes; `:writes` is the pool's per-instance count. Frame 1: two draw items, both resident → changed? true, writes 2. Frame 2: the same draw items → changed? false, writes 0. Frame 3: draw item one re-written with a new revision and the same content → changed? true, writes 1 because the reminted revision changes one positional pool item even though its packed image words are unchanged. Frame 4: the same draw item; a texture lands for its digest after it was a placeholder (the residency revision bumps) → changed? true, writes 1 because that one prepared item changes to the texture binding, and its binding is the texture's. Wrong build that passes a weaker test: a key without the residency revision reports changed? false on frame 4 and draws the placeholder forever; a deep-compare dirty check reports changed? false on frame 3; a return that reshapes the pool's count to a contract number (the first build's `(if (pos? n) 1 0)`).

**I4 — the shared color law paints the same bytes.** Render test harness. The 21 image goldens (`imageAtomCases`) byte-identical in the default mode; `run-color-evidence!`'s linear-mode rows equal to today's within one 8-bit step per channel (the multiply association, `(s·t)·α` against `s·(t·α)`); anything larger is a wrong build. Wrong build that passes a weaker test: `scene_color(vec4(sampled.rgb * tint.rgb, …), cg)`, which linearizes an already-linear sample; the linear-mode rows diverge by far more than a step.
...
**R1 — the hierarchy runs.** Render test harness golden, the first region golden on disk. The `region3d-boundary-fixture` region drawn with `:container 17` under `[0.5 0 0 0.5 40 20]`: its composite quad at half size offset by (40, 20), the surround paths unchanged. An unknown group rejected before upload. Wrong build that passes a weaker test: `composite-row-bytes` writes the cid; it passes while cid equals buffer index.

**R2 — rows travel and the schema rejects by name.** JVM. The `region3d-boundary-fixture` region round-trips through EDN with `nil` metadata, the same canonical map, and the same `:instances` and BVH from `derive-scene`. Rejections by name: `:future/shape` at the top level (`:schema/unknown-key`); a parent cycle (`:region/parent-cycle`, with `:cycle-at` in `ex-data`); a parent that does not exist; a spot light with inner > outer; a torus with tube ≥ radius; an index ≥ vertex count; a quaternion of length 1.01; `:cast-shadow true` on a point light; a placed text with an extra key; `:region3d/version 3`. Wrong build that passes a weaker test: canonicalization normalizing every quaternion, so the 1.01 case passes; the tolerance rule catches it.
...
**R5 — ledgers gone, evidence from returns.** The S5 lifecycle and lower-resolution evidence pass, built from `prepare-region3d-frame!`'s returns; `grep -n "receipt" src/app/client/region3d/renderer.cljs src/app/client/region3d/on_plane_renderer.cljs` is empty.

**T1 — the hierarchy runs.** Render test harness evidence through the slug section: the mixed-face case with its template at `:container 17` under the affine; the glyphs at half size offset by (40, 20). Pinned now as the deterministic tree hash plus the placement check; pinned as a golden PNG at package close, when the last closer adds its runner row (§4d). An unknown group rejected before upload. Wrong build that passes a weaker test: `pack-draw-item!` writing the cid into word 24.

**T2 — diagnostics are returns.** The F3 consistency check reads `:fallbacks` from `pack-instances-flat`'s return: carried draw items 0, uncarried 1 per draw item, as today (`core.cljs:2690-2693`). `grep -rn "__softland" src/app/client/text/renderer.cljs src/app/client/text/glyph_pack.cljs` is empty.
...
- A kind session never edits `harness/core.cljs`, `harness/shared.cljs`, `shadow-cljs.edn`, another kind's files, or `run_verifier.mjs` beyond F7's one block.
...
- **F2, region's open extension.** Default: closed. The row boundary's structural tier says no unknown key rides; `component_test.clj:81-83` flips to a rejection. Sid reopens it in one line if he wants it open.
...
- **F9, the render test harness's shared helpers.** Default: `harness/shared.cljs`; helpers in core would make the kinds require the entry.
