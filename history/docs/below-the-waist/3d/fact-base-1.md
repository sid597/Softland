...
## Part 6 — the fold of 2026-09-06, about 02:30 (what was read, what moved)

Sid carried two contributions to this chair by pasting them: the Codex lane's `3d-kind-working-model.md` (42 KB, read in full from disk) and the object-space contribution `3d-object-space-and-query.md` with `HANDOFF-object-space-and-query.md` (read in full). Also read: `path-kind/HANDOFF-9.md` §6, Sid's late words of about 02:00 (verbatim on the page's ledger). Not read past its header: session 2's `3d-kind-2.html`, `3d-kind-2.md`, `fact-base-2.md`, `bench-2/`; the path kind's `attack-2.md`.

Folded into `3d-kind.html`, with who found it, in the section "Folded in from the two lanes": the definition/occurrence split and the level table; three crossings (present, embed, derive); "surface" as three things (face, surface, target); physics scope apart from grouping; a gesture leaving the face; hover as a query re-run on scene change, emitting diffs, with the stationary-pointer case; the zoom-to-enter transition as data; scheduling (progress, cancellation, versions, explicit state and time); the four differences split into degree and kind; the merge caveat; "difficulty does not prove permanence, native kernels the practical default"; the three-verb interface; the extension question; "retain the description that owns the intended edits, with correspondence back"; the mesh is the leak only when it replaces needed information; query modes and candidates; tolerance-aware CAD; sections keep their conics; coverage declares its interpretation; the screenshot test; the topology-correspondence question and its construction.

Corrected on the page by Sid's words: Position 10, from "the executor is the centre of the waist" to "thin at the centre of the data story; the libraries are thick; constraints are a solver library; throughput lives in the libraries and on the GPU" ("one executor idk seems like the constraints and hard part would be getting them in line throughput seems small....").

Held against the lanes, with reasons on the page: the order rule as the 2D/3D distinction; page zoom as focal length by default; forward with MSAA as the road from here; render to surface as a value (now the Present verb's output).

Bench 0 gained the stationary-pointer case: keys orbit and slide the sphere along a line that crosses the default pointer's ray; the chain re-runs on the key, and the headless probe shows the chain switching from "box · face +z · stroke A: inside" to "sphere · t 2.75" with the GPU id read agreeing, no pointer move (SwiftShader, 2026-09-06).

## Part 7 — the fold of the two reviews of 6f62711 (session 1's successor, 2026-09-06, about 02:45)

Sid carried two reviews of the page and bench as committed at `6f62711`: `feedback-6f62711.md` (the standalone feedback) and `review-6f62711.md` (the review with its two bench receipts), and pasted a longer reading of the same commit into the chair. All three read in full. Both files were written against the page before the two lanes were folded at `bd57eee`; what they asked for that the fold had already done (the stationary-pointer join, "surface" as three things, query modes, the screenshot test) is noted as met, the rest is folded. Nothing in them ranks. Not read past their headers, still: session 2's `3d-kind-2.html`, `3d-kind-2.md`, `fact-base-2.md`, `bench-2/`; `feedback-a85fd1e.md` and `feedback-working-model.md`, which are addressed to other chairs; the path kind's `attack-2.md`.

### What this chair read itself, to check the two source corrections (CHECKED, own read)

- `region3d/renderer.cljs:1046-1064` — `pixel-size` is `ceil(w·zoom·dpr)`, `lease-size` quantises it; `encode-scale` and `encode-bucket` come from `(* zoom dpr)`; `encode-pixel-size` is `ceil(w·encode-scale)`.
- `renderer.cljs:1086-1094` — `view` is `(or (:view session-row) (:view raw-region))`; `view-key` is `[view display-mode encode-bucket shadow-space]`; `camera` is `(scene/camera-matrices view encode-pixel-size)` only when `view-changed?`. So page zoom reaches the interior as the lease size, the encode viewport's aspect and the bucket in the camera's key, and never as the authored lens or pose. The page's "nothing of the page camera reaches the interior" was too broad, as the review said; the fact base's own Part 2 §4 already had the carve-out.
- `renderer.cljs:1134-1139` — `dirty-by-role` is `{:shadow (or scene-changed? (nil? old)) :interior (or scene-changed? view-changed? background-changed? (:changed? placement-return) (nil? old))}`. A camera-only orbit sets `view-changed?` and not `scene-changed?`, so the shadow role stays clean.
- `renderer.cljs:1324-1345` — `encode-region-pass!` encodes a role only when it is dirty or the lease key changed, and otherwise returns `:held? true`. The page's "the interior and the shadow are one dirty region: both re-encode" was wrong; it was derived from the harness looping both roles (Part 2 §3), not from the dirty flags, which the hunter had not recorded.
- `scene.cljc:1088-1125` — `evaluate-scene` classifies the change as `:full` (static component differs), `:none` (transforms equal) or `:transform`; `scene-changed?` on the renderer side is `(not= :none update-kind)`. So the shadow's key is any scene change at the region's grain, not the light and its casters: moving a thing that casts nothing still re-encodes the shadow. That half of the page's claim stands.

### What moved on the page

Recorded on the page under "Folded from the two reviews of 6f62711": the render answer's request names its quantity (contract block, Position 4, the attack table, the surface row); the chart is coordinates plus a mapping onto a face, depth from the mapping (words block, story 7, drawing 1's labels and caption, Position 7, the four-things table, the chart row); "never a biased decal" as the coating's declaration (Position 7, the today table); page zoom as magnification of the placed view, not the region camera's focal length, with lens zoom inside the aperture as the third operation (story 8, moment 2, the cameras answer, Position 5, the open question, the drawing's region label); one waist argued from the six shared means, the chart as evidence only (story 15, Position 11); the opaque case named in story 1; the executor's inputs beyond edits (caption, story 13, Position 10); two tolerances in the value with exact geometry authoritative upstream; the two source corrections above (rate table, moment 2 and 3, the today table); the chart-seam precursor before the boolean split (the definition item, the chart row). The open question is not re-asked; its words are corrected.

### Bench 0, this fold

The wheel's `focal` mode is renamed `magnify` (the old hash value still reads); a `lens` mode changes C1's field of view inside the fixed aperture; the counters read out the aperture in page pixels, C1's lens, the effective focal scale on the screen `(aperture height / 2) / tan(fov / 2)`, and C1's distance, so the three operations show which quantity each changes. The fov rides in the scene key, the surface execution's interior key and the hash. Headless dump on SwiftShader, five states: default (aperture 640×440, lens 45°, focal 531 px, ray d −0.578 −0.395 −0.714, box front face, stroke A inside, id agrees); `zoom=2` (aperture 1280×880, lens 45°, focal 1062, the same ray, footprint halved, id agrees: the review's camera receipt reproduced); `wheel=lens&fov=22.5` (aperture 640×440, lens 22.5°, focal 1106, ray d −0.533 −0.374 −0.759, the box's right face, id agrees); `wheel=dolly&dist=2.6` (aperture, lens and focal unchanged, ray origin moved, id agrees); `wheel=focal` reads as magnify. One screenshot of the lens state: same aperture, the box fills more of it.
