...
0. (Added about 03:40 by the chair after the successor.) Sid asked which line folds which. This chair read session 2's page at a0f20b1 in full and its position is that session 2's line is the trunk and this page folds into it: session 2 already folded this page with a fair record, worked the boolean split under the stroke on its bench (item 1 below, done there as Position 10), and its frame opens from Sid's words where this one opens from the pixel question. The bridge is `crossread-a0f20b1.md`: List A is what this page and bench 0 hold that the trunk lacks, List B the inverse. Bench 0 stays as the receipt for the shadow's key and the three wheel operations. Sid's word closes the direction and who does the fold; until it comes, nothing below is started on this line.
1. Fold session 2's page when Sid carries it (superseded by item 0 if Sid takes the position there); the definer's construction when it lands, in the order the reviews set: the chart-seam precursor (a stroke across a parameterization seam on a curved mesh, reparameterized without changing the surface) and then the boolean split (extruded sketch, attached stroke, split, ink back as a drawing, queried from both views; session 2's bench has the split, not the precursor); nothing to rank. The two reviews of 6f62711 are folded.
