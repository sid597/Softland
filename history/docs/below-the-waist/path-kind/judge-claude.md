...
This card is frozen at the two exchanges of 2026-09-06. The production ground it fed (the roles, the order, the first slice) is in `../two-chairs.md`, the production section, under Sid's rulings with the time; it does not change here.
...
- What holds: the four levels of the data flow as measured (a colour edit writes one row and reruns nothing; a pan and a zoom inside the bucket touch nothing; a rescaled group repacks once; the border reruns per pan because it declared it reads the view); the fragment's local position from its own pixel through the flat inverse (fractional placement within 0.002 of the CPU twin); text byte-identical; the pure files hold nothing; five atoms with one owner each in the renderer. The atlas, the buckets and the instanced draw exist and work; under Sid's later rule (two-chairs, the production section) they are not a credit until a trace shows the need, and the observed-read key was caching built as the design, which is the order the rule forbids.
...
- **What changes with these marks:** the roles for the rounds after. The receipts' reading (the Codex lane the stronger geometer and the weaker integrator; the Claude lane the stronger integrator and verifier that carried the bench's approximations in untested) fed Sid's rulings on 2026-09-06; the ruling itself, the order and the first slice are in `../two-chairs.md`, the production section, and not here.

## 9. After the marks: the two judges, the worker lens, where it went

Read after my marks were set, then two of mine lowered on the Codex judge's receipts, then its file corrected on mine; resolved on code or an executed example throughout, never by vote.

**Verified here and folded in from `judge-codex.md`.** F1, the construction edit the Claude run cache ignores (my A5: `rerun?` false on the pure layer; its browser capture 1 instance retained against 24 fresh). F3, the Codex packer reading `:camera` from a row that carries `:flags` (my C4: `projected-scale` 2.0 at zoom 2 for a screen group). Its R2, a recipe that doubles a width arithmetically and clips with the result, which the Claude executor cannot express. Its input-boundary claim, checked and conceded: the Claude schema rejects a record whose source is a path value or a network even with its own construction, while the executor runs the same record and returns a region (`probes/probe-boundary-claude.out`); the clip already accepts a path value, so the coupling is one validation line, and it is the line to relax before the record format is called the waist. Its size count agrees in shape with mine. Claude row 1 went from 2 to 1 and row 2 from 2 to 1 for A5; Codex row 2 from 1 to 2 for R2.

**Verified there from this file.** C1, the placed ink half missing: the Codex judge reproduced it on its own capture (22 pixels at most 68 bytes falling to 4 at most 6 with the viewport experiment; the repo verifier failing) and corrected its card, its F4. It corrected my wording in return: the fence reading that left `test/` untouched was legitimate, and "routed around a check" claims an intent the tree cannot show; what the tree shows is a red left unreported, and a green flag that reads no pixel. Taken.

**Where the files still differ, on words not facts.** Row 2 Codex, its 3 "within implemented capabilities" against my 2 (the stroke geometry is not a step a record can name; unknown fields pass in silence). Row 3, its 0 against my 1, a reading of the scale's "fails" against "partial". Row 4, its ? against my 2, which was executed here (section 3). Sid reads the receipts either way.

**The worker lens** (Sid's reframing: the delivery is evidence about the builder, not only about the product).

| Dimension | What the trees show | Both judges' view |
|---|---|---|
| Divergence under the same words | different nib construction, executor language, resource model, fence reading, doc depth, verification stance; convergence on the three data lifetimes, no reactive code inside the kind, the fragment fix, the deletion, the crossing numbers | the picture transferred; the engineering did not |
| Communicating the whole design | Claude: a handoff with reasons, compromises, guesses and doubts; Codex: a precise README and a commit message, no doubts stated | Claude, for the written account; neither judge saw the sessions |
| Maintaining the hierarchy | Codex: one docstring discipline, limits and commands inside the tree, and no word that the repo's checks were red; Claude: a deeper hierarchy plus two accounts outside it to keep in step, and claims that outran the code in three named places | Codex by a little inside the tree, weakened by the unreported red |
| The reactive question | both: none inside the kind, scheduling at the caller's edit and frame boundary; Claude's four levels are a dependency design with the program missed from its key; Codex's declared keys by value with the construction left to the caller and one input contract missed | Claude answered more and tripped on its reach; Codex answered the boundary and tripped on a smaller step |
| Purity | both: pure `cljc`, effect owners with lifetimes and cleanup on the error path; Claude's executor stamps a clock; Codex's shader wrapper edits the shared shader by string replacement | both sound; both small fixes |
| Finishing scope | Claude carried most of the bench's vocabulary and every caller; Codex a narrower vocabulary, said so, and one caller half working | Claude; disclosure is not completion |
| Decisions that leave the future free | Codex: the path-value input, keys by value, an executor with arithmetic and a loop, a swappable construction; against it, a second verification surface and silently ignored fields. Claude: the levels, dependency by observation, no compositor change; against it, the source-kind gate, a second language for width rules, a hashed key, and the memo built as the design | split: the public boundary to Codex, the private machinery to Claude, and the machinery earns nothing until measured |
| Self-report against the code | Claude's account over-reaches; Codex's under-reports | two reflexes, one per family, both cheap to police once named. Sid: "fable tries to make shortcuts and codex only does what is told" |

**Where it went.** Both judges' first staffing view (Claude as the accountable builder, Codex on bounded value-layer work and checks) was superseded the same afternoon by Sid's rulings: caching is never an option but a layer on top of a computation that is already as it should be; there is no trial; and the reflexes go where each is a virtue, Fable at max designing as data, Codex at high landing what the design says, the definer against it, a judge of each family once at the slice's end. The order, the chairs, the first slice and the fence sentence are in `../two-chairs.md`. The worker-level open questions both files carry: for Claude, whether it can keep its breadth and explanation while making the public waist independent of today's sources and keeping its claims accurate; for Codex, whether it can complete every requested behaviour and caller, reconcile the existing checks and report remaining failures plainly while keeping its value boundary and economy. Builder usage, effort and elapsed cost remain in Sid's sessions.
