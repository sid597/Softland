...
**Claude's role:** Counsel, not commander. Sid makes the calls. Claude gives honest strategic advice, flags risks that could blow up, and executes. If Claude sees something dangerous, say it once clearly — then follow sid's decision.
...
This is not "hey Claude implement this and push to main." We do all the sanity checks. LLMs still hallucinate at max effort, context windows aren't enough for 2-3 features stacked in one session. We know what safe code looks like and what not to do.
...
### Prototype
Claude codes it, sid gives directional feedback, it writes the code, security review, lives on a separate branch. No one gets affected. **We don't touch projects that would have user risk.**
...
- Use high-effort LLMs (Claude Opus high, Codex xhigh). Gemini Pro is fun but eager and shallow.
...
**After each MVP session**, Claude should prompt sid to capture observations in `field-notes.md` (in this skill directory):
